...
interface General {
  ...
  voiceSilenceMinDuration: number
  alwaysListening: boolean
  audioBufferEnabled: boolean
  audioBufferDuration: number
  interruptOnSpeechDetected: boolean
}
...
const settingsStore = create<SettingsState>()(
  persist(
    (set, get) => ({
      ...
      selectedVrmPath:
        process.env.NEXT_PUBLIC_SELECTED_VRM_PATH || '/vrm/hikari.vrm',
      selectedLive2DPath:
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        voiceOnlyMode: state.voiceOnlyMode,
        vadSensitivity: state.vadSensitivity,
        useVad: state.useVad,
        vadSpeechThreshold: state.vadSpeechThreshold,
        vadSilenceThreshold: state.vadSilenceThreshold,
        showAudioDebug: state.showAudioDebug,
        voiceSilenceMinDuration: state.voiceSilenceMinDuration,
        alwaysListening: state.alwaysListening,
        audioBufferEnabled: state.audioBufferEnabled,
        audioBufferDuration: state.audioBufferDuration,
        interruptOnSpeechDetected: state.interruptOnSpeechDetected,
      }),
    }
  )
)
